...
;; xeno.by: implemented from scratch
(tree-buffer-defpopup-command ecb-rename-source-path
  "Rename this source-path via popup."
  (let ((parent (tree-node->parent node)))
  (let ((file (tree-node->data node)))
  (let ((old-name (tree-node->displayed-name node)))
  (let ((new-name (read-from-minibuffer "New name: " old-name)))
    ;; xeno.by: to be implemented
    (message (concat "rename source-path from \"" old-name "\" to \"" new-name "\""))
    (if (y-or-n-p "Rename source-path also for future-sessions? ")
      (ecb-customize-save-variable 'ecb-source-path ecb-source-path)
      (customize-set-variable 'ecb-source-path ecb-source-path)))))))
...
;; xeno.by: do not add the file to VCS immediately
;; my workflow is magit-oriented, i.e. I'd like to have flexibility
;; of choosing what to stage and what to ignore
(defun ecb-create-source-on-disk (name)
  ...
  (let* ((use-dialog-box nil)
         (dir (ecb-fix-filename
               (funcall (if (ecb-file-directory-p (tree-node->data node))
                        ...
                        (tree-node->data node))))
         (parent (tree-buffer-find-displayed-node-by-data/name dir))
         (filename (ecb-file-name-nondirectory
                     ;; xeno.by: commented line does not work when invoked from popup menu
                     ;; it also does not work when ecb-create-source is called from another command handler
                     ;;(read-file-name "Source name: " (concat dir "/")))))
                     (read-from-minibuffer "Source name: "))))
    (ecb-select-edit-window)
    ;; xeno.by: added integration with VC framework
    ;;(if (save-match-data (string-match "\\.java$" filename))
    ;;    (ecb-jde-gen-class-buffer dir filename)
    ;;  (find-file (concat dir "/" filename)))
    (ecb-create-source-on-disk (concat dir "/" filename))
    ...
    ;; xeno.by: added explicit refresh, since otherwise created file doesn't show up in the tree
    (ecb-goto-window-directories) ;; this call is very important - all manipulations with a tree must be performed in the context of its buffer
    (ecb-update-directory-node parent)
    (tree-buffer-update)
    (ecb-set-selected-source (concat dir "/" filename) 1 nil)
    (ecb-select-edit-window)
...
;; xeno.by: do not add the directory to VCS immediately
;; my workflow is magit-oriented, i.e. I'd like to have flexibility
;; of choosing what to stage and what to ignore
(defun ecb-create-directory-on-disk (name)
  (make-directory dir))


(defun ecb-create-directory (parent-node)
  (let ((dir (concat (tree-node->data parent-node) "/"
                          (read-from-minibuffer "Directory name: "))))
    ;; xeno.by: added integration with VC framework
    ;;(make-directory dir)
    (ecb-create-directory-on-disk dir)
    (ecb-update-directory-node parent-node)
    (tree-buffer-update)

    ;; xeno.by: updated to select created directory after creation
    (ecb-expand-directory-tree dir parent-node)
    (ecb-set-selected-directory dir)

    ;; xeno.by: it's very important to expand parent-node first, since otherwise node will be nil
    (let ((node (tree-buffer-find-displayed-node-by-data/name dir)))
      (let ((line (count-lines 1 (tree-buffer-get-node-name-start-point node))))
        (tree-buffer-recenter node (selected-window))
        (tree-buffer-goto-line line)
        (beginning-of-line)
        (re-search-forward tree-buffer-incr-searchpattern-indent-prefix nil t)))))


(defun ecb-rename-directory-on-disk (old-name new-name)
...
;; xeno.by: implemented from scratch
(tree-buffer-defpopup-command ecb-rename-directory
  ...
  ;;               (ecb-confirm (concat "Directory " new-name-name " exists. Really rename? ")))))
  ...
  (when ok
    ...
    (let ((node (tree-buffer-find-displayed-node-by-data/name new-name)))
    (let ((line (count-lines 1 (tree-buffer-get-node-name-start-point node))))
      (tree-buffer-recenter node (selected-window))
      (tree-buffer-goto-line line)
      (beginning-of-line)
...
;; my workflow is magit-oriented, i.e. I'd like to have flexibility
;; of choosing what to stage and what to ignore
(defun ecb-delete-directory-on-disk (name)
...
(tree-buffer-defpopup-command ecb-delete-directory
  ...
  (let ((dir (tree-node->data node)))
    (when (ecb-confirm (concat "Really delete directory" dir "? "))
      ;; xeno.by: also kill all buffers that point to files that are to be deleted so that we're consistent with the behaviour of ecb-delete-source
          ...
          (with-current-buffer b
            (let ((file (buffer-file-name)))
              (if (and (stringp file)
                ...
                (kill-buffer))))))
      ;; xeno.by: added integration with VC framework
      ;; xeno.by: made delete-directory work recursively (that "t" in the end of the invocation)
      ;;(delete-directory (tree-node->data node) t)
      (ecb-delete-directory-on-disk (tree-node->data node))
...
(setq ecb-common-directories-menu
	...
	(ecb-rename-directory "Rename Directory")
...
(setq ecb-source-path-menu
      (append
       ecb-common-directories-menu
       '((ecb-rename-source-path "Rename Source Path")
         (ecb-delete-source-path "Delete Source Path")
         ("---")
         (ecb-maximize-ecb-window-menu-wrapper "Maximize window"))))


(defun ecb-rename-source-on-disk (old-name new-name)
  (let ((first-existing-parent (file-name-directory (directory-file-name new-name))))
    (loop while (not (file-exists-p first-existing-parent)) do
      (setq first-existing-parent (file-name-directory (directory-file-name first-existing-parent))))
    (when (not (string= first-existing-parent (file-name-directory (directory-file-name new-name))))
...
;; xeno.by: implemented from scratch
(tree-buffer-defpopup-command ecb-rename-source
  "Renames current sourcefile."
  (let ((parent (tree-node->parent node)))
  (let ((file (tree-node->data node)))
  (let ((old-name (if (string= "/" (substring file (- (length file) 1) (length file))) (substring file 0 (- (length file) 1)) file)))
  (let ((old-name-dir (file-name-directory old-name)))
  ...
  ;; that's why I just crash and let the user handle the conflict by him/herself
  ;; (let ((ok (or (not (file-exists-p new-name))
  ;;               (ecb-confirm (concat "File " new-name-name " exists. Really rename? ")))))
...
;; my workflow is magit-oriented, i.e. I'd like to have flexibility
;; of choosing what to stage and what to ignore
(defun ecb-delete-source-on-disk (name)
  (delete-file name))
...
(tree-buffer-defpopup-command ecb-delete-source
  ...
  (let* ((file (ecb-source-get-filename (tree-node->data node)))
    ...
    (when (ecb-confirm (concat "Really delete " (ecb-file-name-nondirectory file) "? "))
      ...
      ;; xeno.by: added integration with VC framework
      ;; (delete-file file)
      (ecb-delete-source-on-disk file)
      (ecb-remove-dir-from-caches dir)
      (ecb-set-selected-directory dir t)
      ;; xeno.by: added explicit refresh, since otherwise deleted file remains in the tree
      (ecb-update-directory-node parent)
      (tree-buffer-update))))
...
(setq ecb-sources-menu
      '(("Grep"
        ...
        ("---")        
	(ecb-create-source "Create Sourcefile")
        (ecb-rename-source "Rename Sourcefile")
